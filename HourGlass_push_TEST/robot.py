...
class Robot(object):
    def __init__(self, is_sim, base_directory, obj_mesh_dir, num_obj, workspace_limits,
        ...
        if self.is_sim:
            ...
            self.sim_client = vrep.simxStart(ud.vrep_ip, ud.vrep_port, True, True, 5000, 5) # Connect to V-REP on a given port
            vrep.simxSetBooleanParameter(self.sim_client, vrep.sim_boolparam_display_enabled, False, vrep.simx_opmode_oneshot)
            ...
            self.training_preset_cases = training_preset_cases
            self.training_preset_folder = training_preset_folder
            self.training_scenario_index = 0
    ...
    def add_objects(self):
        ...
        self.obj_mesh_color = self.color_space[np.random.randint(0, 10, self.num_obj), :]

        # If training with predefined scenarios, read object meshes and poses from training case files
        if self.is_training and self.training_preset_cases:
            current_file = self.training_preset_folder + 'my_training_case_%d.txt'%self.training_scenario_index
            print("training scenario: ", current_file)
            file = open(current_file, 'r')
            file_content = file.readlines()
            self.training_obj_mesh_files = []
            self.training_obj_mesh_colors = []
            self.training_obj_positions = []
            self.training_obj_orientations = []
            for object_idx in range(self.num_obj):
                file_content_curr_object = file_content[object_idx].split()
                self.training_obj_mesh_files.append(os.path.join(self.obj_mesh_dir, file_content_curr_object[0]))
                self.training_obj_mesh_colors.append(
                    [float(file_content_curr_object[1]), float(file_content_curr_object[2]),
                     float(file_content_curr_object[3])])
                self.training_obj_positions.append([float(file_content_curr_object[4]), float(file_content_curr_object[5]),
                                                float(file_content_curr_object[6])])
                self.training_obj_orientations.append(
                    [float(file_content_curr_object[7]), float(file_content_curr_object[8]),
                     float(file_content_curr_object[9])])
            file.close()
            self.obj_mesh_color = np.asarray(self.training_obj_mesh_colors)
        ...
        for object_idx in range(len(self.obj_mesh_ind)):
            ...
            if self.is_training and self.training_preset_cases:
                curr_mesh_file = self.training_obj_mesh_files[object_idx]
            if self.is_testing and self.test_preset_cases:
            ...
            if self.is_training and self.training_preset_cases:
                object_position = [self.training_obj_positions[object_idx][0], self.training_obj_positions[object_idx][1],
                                   self.training_obj_positions[object_idx][2]]
                object_orientation = [self.training_obj_orientations[object_idx][0],
                                      self.training_obj_orientations[object_idx][1],
                                      self.training_obj_orientations[object_idx][2]]
            if self.is_testing and self.test_preset_cases:
                object_position = [self.test_obj_positions[object_idx][0], self.test_obj_positions[object_idx][1],
                                   self.test_obj_positions[object_idx][2]-0.7]
            ...
            if not (self.is_testing and self.test_preset_cases) and not (self.is_training and self.training_preset_cases):
                time.sleep(2)
        self.prev_obj_positions = []
        self.obj_positions = []

        self.training_scenario_index += 1
    ...
    def restart_sim(self):
        ...
        vrep.simxSetObjectPosition(self.sim_client, self.UR5_target_handle, -1, (-0.5, 0, 1),
        ...
        while gripper_position[2] > 0.4+0.7: # V-REP bug requiring multiple starts and stops to restart
            vrep.simxStopSimulation(self.sim_client, vrep.simx_opmode_blocking)
            vrep.simxStartSimulation(self.sim_client, vrep.simx_opmode_blocking)
            time.sleep(1)
            sim_ret, gripper_position = vrep.simxGetObjectPosition(self.sim_client, self.RG2_tip_handle, -1, vrep.simx_opmode_blocking)
    ...
    def get_obj_velocities(self):

        obj_linear_velocities = []
        obj_angular_velocities = []
        for object_handle in self.object_handles:
            sim_ret, obj_linear_velocity, obj_angular_velocity = vrep.simxGetObjectVelocity(self.sim_client, object_handle, vrep.simx_opmode_blocking)
            obj_linear_velocities.append(obj_linear_velocity)
            obj_angular_velocities.append(obj_angular_velocity)

        return obj_linear_velocities, obj_angular_velocities


    def remove_object(self, desired_object_handle):
        # vrep.simxSetObjectPosition(self.sim_client, desired_object_handle, -1, (-0.5, 1.0 + 0.05 * float(0.2), 0.2), vrep.simx_opmode_blocking)
        vrep.simxRemoveObject(self.sim_client, desired_object_handle, vrep.simx_opmode_blocking)
        self.object_handles.remove(desired_object_handle)
        print("Removing object ", desired_object_handle)
    ...
    def reposition_objects(self, workspace_limits):
        ...
        for object_handle in self.object_handles:
            ...
            drop_x = self.xy_positions[index, 0]
            drop_y = self.xy_positions[index, 1]

            object_position = [drop_x, drop_y, 0.15]
            #object_orientation = [2*np.pi*np.random.random_sample(), 2*np.pi*np.random.random_sample(), 2*np.pi*np.random.random_sample()]

            object_orientation = [self.orientations[index, 0], self.orientations[index, 1], self.orientations[index, 2]]
    ...
    def close_gripper(self, async_flag=False):
    ...
    def open_gripper(self, async_flag=False):
    ...
    def push(self, position, heightmap_rotation_angle, workspace_limits):
        ...
        if self.is_sim:
            ...
            if math.isnan(move_direction[0]/move_step[0]):
                num_move_steps = 1
            else:
                num_move_steps = int(np.floor(move_direction[0]/move_step[0]))
    ...
    # def display_q_value(self, image_directory, image):  # NOT WORKING
    #     # sim_ret_q_value, self.cam_handle_q_value = vrep.simxGetObjectHandle(self.sim_client, 'Vision_sensor_q_value',
    #     #                                                     vrep.simx_opmode_blocking)
    #     # vrep.simxSetVisionSensorImage(self.sim_client, self.cam_handle_q_value, *image, 1, vrep.simx_opmode_oneshot)
    #     vrep.simxCallScriptFunction(self.sim_client, 'remoteApiCommandServer', vrep.sim_scripttype_childscript,
    #                                 'display_q_value', [0], [0.],
    #                                 [os.path.join(image_directory, image)], bytearray(), vrep.simx_opmode_blocking)