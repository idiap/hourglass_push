...
class ResidualBlock(nn.Module):
    def __init__(self, input_channels, output_channels):  # input_channels is the number of input_channels
        ...
        layers=[nn.ReplicationPad2d(0),
          ...
          nn.ReLU(inplace=True),  # inplace=True modifies the input directly, without allocating any new memory
...
#             print('[INFO] ({}) {}: {}'.format(thisname, k, v))
    ...
    def make_skip_connection_(self, n_input):  # before each maxpooling, there is a branch
                                               # that goes to 3 residual blocks
    ...
    def forward(self, x):  # x: input image
        ...
        #### upsamplings
        up1 = F.interpolate(Z, scale_factor=2)  # By default, mode='nearest'.
...
class PushingSuccessPredictor(nn.Module):
    def __init__(self, params):
        super(PushingSuccessPredictor, self).__init__()
        thisname = self.__class__.__name__
        self.params = params
...
#                      nn.MaxPool2d(kernel_size=2, stride=2),
    ...
    def forward(self, x):
        ...
        # print('output of PushingSuccessPredictor: ', out.size())
        
        return out

    
def get_psp_parameters():
    """
    ...
    return {  # channels produced by the first layers before HG
        'front_channels': 16,
        # channels input to the HG blocks
        'hg_input_channels': 32,
        # channels produced across the HG
        'hg_across_channels': 64,
        # how many hourglass modules
        'n_stages': 1,
        # channels of the image
        'input_channels': 1,
